...
class Base_model extends CI_Model
{
    ...
    function get_table()
    {
        return ($this->session->userdata('session_db') == NULL) ? $this->model['table']['name'] :  $this->session->userdata('session_db') . '.' . $this->model['table']['name'];
    }
    ...
    function get_view($dbses = false)
    {
        $viewname = $this->model['view']['name'];
        if (!$dbses) {
            if ($this->session->userdata('session_db') != null || $this->session->userdata('session_db') != '') {
                $viewname = strpos($viewname, '.') ? $viewname : $this->session->userdata('session_db') . '.' . $viewname;
            }
        }
        return $viewname;
    }
    ...
    function read($id = null, $rendered = false, $mapped = true, $untrack = false, $custom_fields = null, $dbses = false)
    {
        ...
        if (!empty($id)) {
            if ($mapped) {
                if ($custom_fields != null) {
                    ...
                    if (is_array($custom_fields)) {
                        if (isAssoc($custom_fields)) {
                            $fields_mapper = $fields = varMatch($custom_fields, $fields_public);
            ...
            $result = $this->db->get_where($this->get_view($dbses), $where);
    ...
    function select($config = null, $fn = null, $db_toko = null)
    {
