<script type="text/javascript">
	...
	function getLaporan() {
		HELPER.block();
		$.ajax({
			url: BASE_URL + 'laporanpembelian/get_laporan' + $('#laporan').val(),
	...
	function getLaporanExcel() {
		...
		HELPER.block();
		$.ajax({
			url: BASE_URL + '/laporanpembelian/spreadsheet_laporan',
			type: 'post',
			data: $('#lap-pembelian').serializeObject(),
			dataType: 'JSON',
			success: function(res) {
				if (res.success) {
					let fileLocation = BASE_ASSETS + 'laporan/laporan_pembelian/' + res.file;
