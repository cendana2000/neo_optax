...
class Loginkasir extends BASE_Controller
{
    ...
    public function index()
    {
        ...
        if ($id == "") {
            $this->load->view('loginkasir/login');
        } else {
            $this->main($id);
        }
    }
            ...
            /*
                ...
                if (!empty($user)) {
                    if ($user['user_is_registered']) {
                        ...
                        $user['session_db'] = 'pospajak_dev';
                        $user['global_pajak'] = 0;
                        $user['toko_nama'] = 'SUPER ADMIN';
                    ...
                    $this->response(array(
                        'success' => false,
                        'message' => 'User not found. Please check your email, password and code store.',
                    ));
                }
                */
        }
            ...
            /*
                ...
                if (!empty($user)) {
                    if ($user['user_is_registered']) {
                        ...
                        $operation['nama'] = $user['toko_nama'];
                        $operation['session_db'] = 'pospajak_dev';
                        $this->session->set_userdata($user);
    ...
    public function main($user_id)
    {
        ...
        // print_r($data);exit;
        // $data_menu = group_by_array($data, 'menu_description');
        $html = '';
        $redirect = 'main/main';
        $firstClick = null;
        $countMenu = 0;
