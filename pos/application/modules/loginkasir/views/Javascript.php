<script type="text/javascript">
	...
	// 	$.post(BASE_URL + "history_login", function(res) {
	// 		if (res.success == true) {
	// 			if (!!cbSuccess) {
	// 				cbSuccess();
	// 			}
	...
	function doLogin() {
		...
		$.ajax({
			type: "POST",
			url: BASE_URL + "loginkasir/doLogin",
			data: {
				toko_kode,
				email: email,
				password: password,
	...
	function handlerTokoLocalStorage(res) {
		if (res.data && res.data.toko) {
			localStorage.setItem("toko", JSON.stringify(res.data.toko));
		}
	}

	function handlerCodeStore() {
		// let toko = null;
		try {
		...
		if (toko) {
			$("#form_codestore").addClass("d-none");
			$("#pengaturan_code_store").val(toko.toko_kode);
			$("#form-pengaturan_hapus").removeClass('d-none');
			$("#info-alert").addClass('d-none');
			$("#info-nama_toko").text(toko.toko_nama);
		} else {
			$("#form_codestore").removeClass("d-none");
			$("#toko_nama").text("");
			$("#form-pengaturan_hapus").addClass('d-none');
	...
	function onSavePengaturan() {
		let codestore = $("#pengaturan_code_store").val();
		HELPER.ajax({
			url: BASE_URL + 'login/gettoko',
			data: {
				toko_kode: codestore,
			},
			success: function(res) {
				if (res.success) {
					...
					toko_kode = codestore;
					localStorage.setItem('toko', JSON.stringify(res.data));
					$("#toko_nama").text("");
					$("#form-pengaturan_hapus").removeClass('d-none');
					$("#info-nama_toko").text(res.data.toko_nama);
					$("#info-alert").addClass('d-none');
					HELPER.showMessage({
						title: "Success",
						success: "success",
	...
	function onDelPengaturan() {
		HELPER.confirm({
			message: 'Apakah anda yakin ingin menghapus pengaturan?',
...
<!-- <script type="module">
	...
	// var socket = io("https://192.168.100.59:3000/");
