...
<script type="text/javascript">
	$(function() {
		...
		HELPER.ajaxCombo({
			el: '#barang_id',
			url: BASE_URL + 'pricelist/select_ajax',
		});
	...
	function print_table_card() {
		...
		HELPER.getDataFromTable({
			...
			callback: function(res) {
				if (res.length > 0) {
					$.ajax({
						url: BASE_URL + 'pricelist/tprint_card',
						data: {
							'table': res,
							'ftype': 'table',
					...
					HELPER.confirm({
						title: 'Konfirmasi',
						message: 'Apakah anda yakin untuk mencetak semua data yang tampil ?',
						callback: function(res) {
							if (res == true) {
								dform = $('#pricelist-form').serializeObject();
								dform = $.extend(dform, {
									'ftype': 'form'
								});
								dform = $.extend(dform, {
