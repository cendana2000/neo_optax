...
class Transaksipenjualan extends Base_Controller
{
	...
	public function load_data_mobile($param = '')
	{
		if (!empty($dbname)) {
	...
	public function tprint($id)
	{
		...
		$metode_pembayaran = '';
		if ($jual['penjualan_metode'] == 'T' || $jual['penjualan_metode'] == 'B') {
		...
		foreach ($detail as $key => $value) {
			...
			$rowData .= '
				<tr>
					<td align="left" class="txt-left">' . $value['barang_nama'] . '<br> <span style="font-size:7px">' . $notes . '</span></td>
					<td align="center" class="txt-center" style="padding-left: 10px;">' . round($value['penjualan_detail_qty']) . '</td>
					<td align="right" class="txt-right" style="padding-left: 10px;">' . $value['barang_harga'] . '</td>
					<td align="right" class="txt-right" style="padding-left: 10px;">' . number_format($value['barang_harga'] * $value['penjualan_detail_qty']) . '</td>
				</tr>
			';
		}

		// print_r('<pre>');print_r($jual);print_r('</pre>');exit;
				...
				<body class="struk" onload="printOut()">
					<section class="sheet">
						<table cellpadding="0" cellspacing="0" style="width:100%">
							...
							<tr>
								<td align="center" class="text-center" style="padding-top: 5px; ' . ($this->config->item('struk_is_antrian') == 'true' ? '' : 'display: none;') . '">Antrian 01</td>
							</tr>
							<tr>
