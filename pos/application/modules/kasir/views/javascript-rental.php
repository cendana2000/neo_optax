...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#listHandler").scroll(function(event) {
			var element = event.target;

			if (isBottomElementVisible($("#listHandler").find(".card").last()[0], $("#listHandler")[0])) {
		...
		$('#receipts').append('<p class="empty-order mx-auto my-auto">Belum ada produk yang dipesan</p>');
	});
                ...
                <div class="card-body p-5">
                    ...
                    <div class="row">
                        <div class="col-8 d-flex flex-column">
                            <span class="font-size-xs d-flex align-items-center mb-0 mt-auto" style="gap: 5px;">
                                ${jenisStok}
                            </span>
	...
	function initOrder({
		...
		if ($('#receipts').find('.empty-order').length > 0) {
			$('#receipts').html('');
		}
		...
		if (flag < 1) { // jika belum ada data dalam table order
						...
						<div class="card-body">
							<div class="row align-items-center">
								<div class="col-5">
									...
									<span class="d-block font-weight-bolder h6 mb-0" id="barang_nama_${row}">${barang_nama}</span>
									<span class="d-block font-weight-bold text-muted font-size-xs">Kode: 213849156</span>
								</div>
								<div class="col-5 d-flex flex-column">
									<span class="font-size-sm text-primary font-weight-bolder">Rp. ${formatRupiah(barang_harga)}/night</span>
								</div>
								<div class="col-2 p-0 text-end">
									<img loading="lazy" class="img-fluid rounded" style="width:50px; height:50px; object-fit:cover;" src="${barang_thumbnail != null ? '<?= base_url() ?>'+barang_thumbnail : ''}" onerror="imgError(this)">
								</div>
							</div>
							...
							<div class="row align-items-center">
								<div class="col-4 d-flex align-items-center" style="gap: 5px;">
									...
									<span>Rp.</span> 
									<input id="penjualan_detail_harga_beli_${row}" type="text" data-row="${row}" data-inpajak="${parseInt(barang_harga *(LSToko.jenis_tarif / 100))}" data-defaultHarga="${barang_harga}"  name="penjualan_detail_harga_beli[${row}]" onkeyup='countPriceFlexiblePrice(${JSON.stringify(orderItem)})' value="${barang_harga}" class="number hargaBeli py-1 px-3 rounded w-100" style="border: 1px solid #E5E5F0;">
								</div>
								<div class="col-3 font-weight-bolder h6 mb-0">
									<select onchange="satuanKonversi(this, ${row}, ${qty}, ${jenis_include_stok})" name="penjualan_detail_satuan[${row}]" id="penjualan_detail_satuan_${row}" class="rounded border-0 py-1" style="background-color: #F7F7F7;"></select>
								</div>
								<div class="col-5 d-flex align-items-center px-0" style="gap: 15px;">
									<div class="d-flex align-items-center" id="updateHarga_${row}" style="gap: 15px;">
										...
										<span class="h5 font-weight-bolder mb-0">
											...
											<span><span id="quantity_${row}">${qty}</span> Night</span>
										</span>
			...
			const quantity = $('#penjualan_detail_qty_barang_' + cIndex).val();
			if (quantity == 2) {
	...
	function loadMenu(page = 1) {
		...
		$.ajax({
			url: BASE_URL + 'transaksipenjualan/load_menu_rental',
			data: {
				valOrder,
				valKategori,
				valSearch,
	...
	function searchMenu() {
		...
		$('#listHandler').empty();
		$('#listHandler').append('<p class="mx-auto my-auto">Mohon tunggu</p>');
		...
		$.ajax({
			...
			success: function(res) {
				...
				if (res.total_count > 0) {
					...
					$('#listHandler').empty();
					$('#listHandler').append('<p class="d-flex align-items-center justify-content-center" style="grid-column: 1 / 5; min-height: 50vh;">Menu tidak ditemukan</p>');
				}
			},
			error: function(res) {
				swal.fire('Pencarian Gagal', 'Waktu pencarian melebihi batas, periksa koneksi anda', 'warning');
				$('#listHandler').empty();
				$('#listHandler').append('<p class="d-flex align-items-center justify-content-center" style="grid-column: 1 / 5; min-height: 50vh;">Timeout</p>');
			}
		});
			...
			HELPER.confirm({
				...
				callback: function(suc) {
					if (suc) {
						...
						if (!$.trim($('#receipts').html())) $('#receipts').append('<p class="empty-order mx-auto my-auto">Belum ada produk yang dipesan</p>');
	...
	function onReset(callbackCombo = null) {
		...
		$('#receipts').html('<p class="empty-order mx-auto my-auto">Belum ada produk yang dipesan</p>');
		$('#customerDiv').empty();
					...
					if (success === true) {
						...
						if (cetak.is(":checked")) {
							...
							if (print) {
								...
								$('#pdf-laporan').html(`<object data="" type="application/pdf" width="100%" height="500px"></object>`);
	...
	function onPrint(param) {
		...
		HELPER.getDataFromTable({
			...
			callback: function(data) {
				...
				$.ajax({
					url: BASE_URL + 'transaksipenjualan/tprint/' + id + '/pdf',
					data: {
						settings_show,
						tjson: true
					},
