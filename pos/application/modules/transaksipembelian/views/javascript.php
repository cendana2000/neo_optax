<script type="text/javascript">
	...
	$(function() {
		...
		HELPER.api = {
			table: BASE_URL + 'transaksipembelian/index2',
		...
		HELPER.createCombo({
			...
			callback: function(res) {
				...
				$('#detail_barang_satuan_satuan_id_1, #barang_satuan_satuan_id_2,#barang_satuan_satuan_id_3').select2();
			}
		})
		...
		loadTable();
		// setRekening();
		$('#pembelian_jatuh_tempo_hari').val(31);
	});
		...
		HELPER.initTable({
			...
			columnDefs: [{
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						let html = ``;
						if (full['pembelian_bayar_opsi'] == "T") {
							html = `<span class="label label-success label-inline mr-2">Data Dapat Diubah</span>`;
						} else {
							if (parseInt(full['pembelian_bayar_jumlah']) > 0) {
								html += `<span class="label label-warning label-inline mr-2">Angsur/Lunas</span>`;
							}

							if (parseInt(full['pembelian_retur']) > 0) {
								html += `<span class="label label-info label-inline mr-2">Terjadi Retur</span>`;
							}

							if (parseInt(full['pembelian_lock']) == 1) {
								html += `<span class="label label-danger label-inline mr-2">Posting</span>`
							}
							if (parseInt(full['pembelian_bayar_jumlah']) <= 0 && parseInt(full['pembelian_retur']) <= 0) {
								if (full['pembelian_lock'] == 0 || full['pembelian_lock'] == null) {
									html = `<span class="label label-success label-inline mr-2">Data Dapat Diubah</span>`;
								}
							}
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						...
						if (full['pembelian_bayar_opsi'] == "T") {
							btn_aksi += `<a href="javascript:;" class="btn btn-sm btn-success btn-icon mx-1" title="Detail" onclick="trigDetail(this)">
							<span class="svg-icon svg-icon-md">
						...
						<span class="svg-icon svg-icon-md">
							...
							} else if (parseInt(full['pembelian_bayar_jumlah']) >= 0 && parseInt(full['pembelian_retur']) >= 0 && parseInt(full['pembelian_lock']) != 0) {
								if (full['pembelian_lock'] != 0 || full['pembelian_lock'] != null) {
									btn_aksi += `<a href="javascript:;" class="btn btn-sm btn-success btn-icon mx-1" title="Detail" onclick="trigDetail(this)">
							<span class="svg-icon svg-icon-md">
								<i class="fa fa-info-circle"></i>
									...
									btn_aksi += `<a href="javascript:;" class="btn btn-sm btn-primary btn-icon mx-1" title="Edit" onclick="Swal.fire('Warning', 'Data tidak dapat diubah', 'error')">
							<span class="svg-icon svg-icon-md">
								<i class="fa fa-pen"></i>
							</span>
							</a>`;
									btn_aksi += `<a href="javascript:;" class="btn btn-sm btn-danger btn-icon mx-1" onclick="Swal.fire('Warning', 'Data tidak dapat dihapus', 'error')">
												<span class="svg-icon svg-icon-md">
													<i class="fa fa-trash"></i>
												</span>
											</a>`;
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						...
						if (full['pembelian_bayar_opsi'] == "T") {
							...
							if (parseInt(full['pembelian_retur']) > 0) {
								html += `<span class="label label-info label-inline mr-2">Retur</span>`;
							}
				...
				$.each(r.data, function(i, v) {
					html = `<tr class="barang_` + n + `">
							<td class="sticky-col scol-1" scope="row">
								...
								<input type="hidden" class="form-control" name="hg_barang_kode[` + n + `]" id="hg_barang_kode_` + n + `" value="` + v.barang_kode + `" style="width:20%;display:inline" disabled>
	...
	function setRekening() {
		trow = '#order_detail_pembayaran_cara_bayar_' + rowPembayaran;
		$.get(BASE_URL + 'rekening/select', function(res) {
			$(trow).empty();
			$(trow).append(`<option value="" selected>-Pilih Pembayaran-</option>`);
			$.each(res.data, function(i, v) {
				$(trow).append(`<option value="${v.rekening_id}">${v.paket}</option>`);
			});
		});
		$(trow).select2();
		$('input.number').number(true);
			...
			columnDefs: [
				...
				},
				{
					...
					render: function(data, type, row) {
                        ...
                        <a href="javascript:;" class="btn btn-light-warning btn-sm" onclick="onHarga('` + dt.pembelian_id + `', '` + row[2] + `')" title="Harga">
                          <span class="la la-money"></span> Harga
                        </a> | 
                        <a href="javascript:;" class="btn btn-light-warning btn-sm" onclick="onDestroy(this)" title="Hapus" >
	...
	function getHarga(row, detail) {
		...
		let pembeliandetailid = $(`#pembelian_detail_id_${row}`).val();
		if (pembeliandetailid) {
			let state_barangdetail_obj = state_barangdetail.find(f => f.pembelian_detail_id = pembeliandetailid);

			if (state_barangdetail_obj.pembelian_detail_konversi > harga.barang_satuan_konversi) {

				$(`#pembelian_detail_qty_${row}`).val((state_barangdetail_obj.pembelian_detail_qty * state_barangdetail_obj.pembelian_detail_konversi) / harga.barang_satuan_konversi);
			} else {
				$(`#pembelian_detail_qty_${row}`).val(state_barangdetail_obj.pembelian_detail_qty);
				$('#pembelian_detail_qty_barang_' + row).val(qty_barang * harga.barang_satuan_konversi);
			}
		}
	...
	function addBarang(reset = 0) {
		...
		html = `<tr class="barang_` + row + `">
					<td scope="row">
						...
						<input type="hidden" class="form-control barcode-scan" style="display:none" name="barang_kode[` + row + `]" id="barang_kode_` + row + `"  style="width:18%;display:inline;" data-id="` + row + `" placeholder="Barcode">
					...
					<td>
						<input class="form-control number qty" type="text" name="pembelian_detail_qty[` + row + `]" id="pembelian_detail_qty_` + row + `" onkeyup="countRow('` + row + `')" value="1">
					...
					<td>							
						<div class="kt-input-icon kt-input-icon--right">
							<input type="text" class="form-control disc" placeholder="%" id="pembelian_detail_diskon_` + row + `" name="pembelian_detail_diskon[` + row + `]" onkeyup="countRow('` + row + `')">
							<span class="kt-input-icon__icon kt-input-icon__icon--right">
							</span>
						</div>	
					</td>
					<td><input class="form-control number jumlah" type="text" name="pembelian_detail_jumlah[` + row + `]" readonly id="pembelian_detail_jumlah_` + row + `" onchange="setHarga('` + row + `')"></td>
					<td>
						<a href="javascript:;" data-id="` + row + `" class="btn btn-light-warning btn-sm" onclick="remRow(this)" id="remLast_${row}" title="Hapus" >
	...
	function addBarangDetail(reset = 0) {
		if (reset == 0) {
			row++;
		}
		html = `<tr class="barang_` + row + `">
		...
		HELPER.loadData({
			...
			callback: function(res) {
				...
				$.each(res.pembayaran, function(i, v) {
					$.post(BASE_URL + 'rekening/get_data', {
						rekening_id: v.order_detail_pembayaran_cara_bayar
					}, function(res) {
						$('#order_detail_pembayaran_cara_bayar_' + rowPembayaran).val("Cash").trigger("change");
				...
				onBayar();
				onAdd();
				let lastRow = res.detail.length + 1;
				$('tr.barang_' + lastRow).remove();
	...
	function trigDetail(el) {
		HELPER.loadData({
			...
			callback: function(res) {
				...
				$("#pembelian_supplier_id2").val(res.supplier_nama);

				if (res.pembelian_bayar_opsi == 'K') {
					$("#pembelian_bayar_opsi2").val('Kredit');
				} else {
					$("#pembelian_bayar_opsi2").val('Tunai');
				...
				$.each(res.detail, function(i, v) {
					let html = `
						<tr>
							<td>${v.barang_nama}</td>
							<td>${v.barang_satuan_kode}</td>
							<td>${v.pembelian_detail_harga_barang}</td>
							<td>${v.pembelian_detail_qty}</td>
							<td>${v.pembelian_detail_diskon}</td>
							<td>${v.pembelian_detail_jumlah}</td>
						</tr>
					`;
	...
	function save() {
		// check supplier if kredit
		let pembelian_bayar_opsi = $('#pembelian_bayar_opsi').val();

		if (pembelian_bayar_opsi == 'K') {
			let pembelian_supplier_id = $('#pembelian_supplier_id').val();

			if (pembelian_supplier_id == '' || pembelian_supplier_id == undefined) {
				swal.fire('Informasi', 'Supplier wajib di pilih saat menggunakan metode pembayaran kredit!', 'warning');
				return;
			}
		}

		// Prevent if produk detail empty
		let flag = {
			countRow: 0,
			emptyRow: 0,
			detailRow: []
		...
		$('.barang_id').each(function(i, v) {
			let cId = $('#' + $(this).attr('id')).val();
			flag.countRow += 1;
			flag.detailRow.push(cId);
		...
		// check if bayar kurang atau lebih
	...
	function addPembayaran() {
		...
		html = `<tr class="pembayaran_` + rowPembayaran + `">
					...
					<td>
						<select class="form-control" name="order_detail_pembayaran_cara_bayar[` + rowPembayaran + `]" id="order_detail_pembayaran_cara_bayar_` + rowPembayaran + `" style="width: 100%" onchange="setBayar()">
							<option value="Cash">Cash</option>
						</select>
					</td>
					<td>
	...
	function jenisBayar() {
		...
		if (choose == 'K') {
			...
			$('#pembelian_jatuh_tempo_hari').val(0);
			jatuhTempo.hide();
			lab_jatuhTempo.hide();
			btnTunai.show(100);
		} else {
