...
class Produk extends Base_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'cahyo_produk/ProdukModel' 				 => 'produk',
			'cahyo_produk/ProduksatuanModel'			 => 'produksatuan',
			'cahyo_produk/ProdukbarcodeModel'			 => 'produkbarcode',
			'cahyo_kategori/KategoriModel' => 'kategori',
		));
	}
	...
	public function select_ajax($value = '')
	{
		...
		$total = $this->db->query('SELECT count(produk_id) total FROM `' . $this->session->userdata('session_db') . '`.pos_produk WHERE ' . $where . ' (barang_nama like "' . $data['q'] . '%" OR produk_kode like "' . $data['q'] . '%") ')->result_array();
		$return = $this->db->query('SELECT produk_id as id, concat(produk_kode, " - ", barang_nama) as text, produk_is_konsinyasi as saved FROM `' . $this->session->userdata('session_db') . '`.v_pos_produk WHERE ' . $where . ' (barang_nama like "' . $data['q'] . '%" OR produk_kode like "' . $data['q'] . '%") ORDER BY barang_nama LIMIT ' . $data['page'] . $data['limit'])->result_array();
	...
	public function store()
	{
		...
		$operation = $this->produk->insert($id, $data, function ($res) use ($detail) {
			$update = $this->db->insert_batch('`'.$this->session->userdata('session_db').'`.pos_barang_satuan', $detail);
	...
	public function update()
	{
		...
		$operation = $this->produk->update(varPost('id', varExist($data, $this->produk->get_primary(true))), $data, function (&$res) use ($detail, $insert_detail) {
			$update = $this->db->update_batch('`'.$this->session->userdata('session_db').'`.pos_produk_satuan', $detail, 'produk_satuan_id');
			// $res_detail = $this->produksatuan->update($data['produk_satuan_id'][$key], $detail);				
			if (!$update) {
				$res['message'] = 'Berhasil menyimpan dengan error[satuan gagal diperbarui]';
			}
			if ($insert_detail) {
				$inset_detail = $this->db->insert_batch('`'.$this->session->userdata('session_db').'`.pos_produk_satuan', $insert_detail);
			}
		});
		$this->response($operation);
	}
	...
	{
		// echo '<p>'.$kategori.'</p>';
		$db_kategori = $this->db->select('kategori_barang_id,kategori_barang_parent')->get_where('`'.$this->session->userdata('session_db').'`.pos_kategori', ['kategori_barang_id' => $kategori])->result_array();
		$parent = [];
	...
	public function list_satuan_harga($value = '')
	{
		...
		$harga = $this->db->select('pos_produk_satuan.*, pos_produk.produk_harga_pokok')
			...
			->join('`'.$this->session->userdata('session_db').'`.pos_produk', 'produk_id = produk_satuan_parent', 'left')
			->order_by('produk_satuan_order')
			->get('`'.$this->session->userdata('session_db').'`.pos_produk_satuan')->result_array();
