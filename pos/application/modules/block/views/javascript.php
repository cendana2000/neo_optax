<script type="text/javascript">
	...
	$(function() {
		...
		HELPER.createCombo({
			...
			callback: function(res) {
				$('#pos_penjualan_customer_id').select2();
			}
		})
	...
	function initMenu(param) {
		...
		if (param.jenis_include_stok == 1) {
			...
			jenisStok = `<span font-weight-bolder" style="font-color:#1bc5bd"><i style="font-color:#1bc5bd" class="fas fa-sm fa-infinity"></i></span>`;
		}
		...
		const html = `
						<div class="col-md-3 p-2">
							...
							role="button"
							onclick="initOrder('${param.id}', '${param.barang_nama}', '${param.barang_harga}', ${param.stok_now}, ${param.jenis_include_stok});">
	...
	function satuanKonversi(param, row, qty, stok_now) { // param stok_now temporary unuse
	...
	function initOrder(id, barang_nama, barang_harga, stok_now, jenis_include_stok) {
		...
		let html = `
			<tr class="order_${row}">
				...
				<td>
				<select class="form-control" onchange="satuanKonversi(this, ${row}, ${qty}, ${stok_now})" name="penjualan_detail_satuan[${row}]" id="penjualan_detail_satuan_${row}">
				<option value="">-Pilih Satuan-</option>
				</select>
				</td>
				<td>
					<input id="penjualan_detail_harga_beli_${row}" type="text" readonly style="background-color: #eaeaea;" name="penjualan_detail_harga_beli[${row}]" value="${barang_harga}" class="form-control number">
				</td>
	...
	function countPrice(cRow, cHarga, s_press = 0, stokNow = 0) { //param stoknow temporary unuse
		...
		if (cQty < 0) {
			...
			$("#total_row_" + cRow).val(`${$.number(cTotal)}`);
	...
	function countPajak() {
		let total = countTotal() + countJasa();
	...
	function remOrder(el) {
		...
		// 	text: "Produk akan hilang dari table!",
		// 	icon: 'warning',
	...
	function onEdit(el) {
		HELPER.loadData({
			...
			inline: $(el),
			callback: function(res) {
				// $('#penjualan_total_bayar').val(parseInt(res.parent.penjualan_total_bayar));
				// $('#pos_penjualan_customer_id').val(res.parent.pos_penjualan_customer_id);
				onReset(() => {
			...
			columnDefs: [{
				...
				},
				{
					...
					render: function(data, type, row) {
						...
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md kt-font-bold kt-font-active" onclick="onDelete('` + data + `')" title="Batal Transaksi" >
                          <span class="la la-times"></span> Batal
                        </a>	
						`;
	...
	function daftarPesanan(id) {
		...
		$.ajax({
			...
			success: (res) => {
				...
				$.each(res, (v, data) => {
					...
					$('#pesanan-masuk').append(`
						<tr>
							...
							<td> ` + (HELPER.toCurrency(data.barang_harga)) + `</td>
							<td> ` + HELPER.toCurrency(data.barang_harga * data.barang_qty) + `</td>
						</tr>
						`);
				...
				$('#subtotal').html(' ' + (HELPER.toCurrency(subtotal)))
