...
class CustomerModel extends Base_Model
{
	public function __construct()
	{
		$model = array(
			...
			'view' => array(
				'mode' => array(
					'table' => array(
						'customer_id', 'customer_kode', 'customer_nama', 'customer_membership', 'customer_created_at',
						'customer_updated_at',
						'customer_deleted_at',
					)
				)
	...
	{
		return $this->db->query("select count(customer_nama) as res from pos_penjualan join pos_customer on pos_penjualan_customer_id = customer_id  where customer_id = '{$data['id']}'")->row_array()['res'];
	}
}
