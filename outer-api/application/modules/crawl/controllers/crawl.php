...
class Crawl extends Main_Controller
{
    ...
    {
        $wps        = $this->data_model->get_settings();
        ...
        foreach ($wps as $wp) {
            ...
            if ($preset_curl == "MAJOO") {
                $this->load->library("etax_crawler/majoo", "majoo");
                $data = $this->majoo->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "MOKA") {
                $this->load->library("etax_crawler/moka", "moka");
                $next_cursor = null;
                $data = $this->moka->get_data($setting, $start_date, $end_date, "");
                ...
                if (count($data) > 0) {
                    ...
                    while ($next_cursor != null) {
                        $ambil_lagi = $this->moka->get_data($setting, $start_date, $end_date, $next_cursor);
                        $data = array_merge($data, $ambil_lagi);
                ...
                $this->load->library("etax_crawler/olsera", "olsera");
                $data = $this->olsera->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "OLSERAV2") {
                $this->load->library("etax_crawler/olserav2", "olserav2");
                $data = $this->olserav2->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "PARAGON") {
                $this->load->library("etax_crawler/paragon", "paragon");
                $data = $this->paragon->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "PAWOON") {
                $this->load->library("etax_crawler/pawoon", "pawoon");
                $data = $this->pawoon->get_data($setting, $start_date, $end_date, 1);

                #jika halaman dari curl sebelumnya > 1 maka ambil data lagi!
                if (count($data) > 0) {
                    $total_page = $data[0]["total_page"];
                    for ($i = 2; $i <= $total_page; $i++) {
                        $data = array_merge($data, $this->pawoon->get_data($setting, $start_date, $end_date, $i));
                    }
                }
            } elseif ($preset_curl == "ESBPOS") {
                $this->load->library("etax_crawler/esbpos", "esbpos");
                $data = $this->esbpos->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "ESBPOSV2") {
                $this->load->library("etax_crawler/esbposv2", "esbposv2");
                $data = $this->esbposv2->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "NUTAPOS") {
                $this->load->library("etax_crawler/nutapos", "nutapos");
                $data = $this->nutapos->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "LARISPOS") {
                $this->load->library("etax_crawler/larispos", "larispos");
                $data = $this->larispos->get_data($setting, $start_date, $end_date, 1);

                #jika halaman dari curl sebelumnya > 1 maka ambil data lagi!
                if (count($data) > 0) {
                    $total_page = $data[0]["total_page"];
                    for ($i = 2; $i <= $total_page; $i++) {
                        $data = array_merge($data, $this->larispos->get_data($setting, $start_date, $end_date, $i));
                    }
                }
            } elseif ($preset_curl == "LARISPOSV2") {
                $this->load->library("etax_crawler/larisposv2", "larisposv2");
                $data = $this->larisposv2->get_data($setting, $start_date, $end_date, 1);
            } elseif ($preset_curl == "LOYVERSE") {
                $this->load->library("etax_crawler/loyverse", "loyverse");
                $data = $this->loyverse->get_data($setting, $start_date, $end_date, 1);
            } elseif ($preset_curl == "KASIRPINTAR") {
                $this->load->library("etax_crawler/kasirpintar", "kasirpintar");
                $data = $this->kasirpintar->get_data($setting, $start_date, $end_date, 1);
            } elseif ($preset_curl == "GOBIZ") {
                $this->load->library("etax_crawler/gobiz", "gobiz");
                $merchandID = json_decode($wp->token_req_payload)->merchant_id;
                $outlet     = json_decode($wp->token_req_payload)->outlet;
                $data       = $this->gobiz->get_data($setting, $start_date, $end_date, 0, $merchandID, $outlet, "");
                ...
                if ($total_halaman > 1) {
                    ...
                    for ($i = 1; $i <= $total_halaman; $i++) {
                        $from = ($i - 1) * $maximumData;
                        $data       = $this->gobiz->get_data($setting, $start_date, $end_date, $from, $merchandID, $outlet, $last_token);
                    }
                }
            } elseif ($preset_curl == "LUNAPOS") {
                $this->load->library("etax_crawler/lunapos", "lunapos");
                $data = $this->lunapos->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "ISELLER") {
                $this->load->library("etax_crawler/iseller", "iseller");
                $data = $this->iseller->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "WOOGIGS") {
                $this->load->library("etax_crawler/woogigs", "woogigs");
                $data = $this->woogigs->get_data($setting, $start_date, $end_date);
            } elseif ($preset_curl == "OMEGAPOS") {
                $this->load->library("etax_crawler/omegapos", "omegapos");
                $data = $this->omegapos->get_data($setting, $start_date, $end_date);
            }

            if (!empty($data)) {
                $this->data_model->store($npwpd, $preset_curl, $data, $start_date, $end_date);
            }
        }
