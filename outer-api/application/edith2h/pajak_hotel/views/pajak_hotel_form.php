    ...
    <div class="controls-row">
      <div class="control-group pull-left" data-bind="validationElement: nospt" >
        ...
        <input type="text" class="span3" id="nospt" data-bind="value: nospt, attr: {readonly: auto()}" required />
	     ...
	     <div class="control-group" data-bind="validationElement: billing">
        <label class="control-label" for="billing">Billing</label>
        <input type="text" class="" id="billing" data-bind="value: billing, attr: {readonly:auto()}"/>
      </div>
...
$(document).ready(function() {
  ...
  var ModelPendaftaran = function (){
	...
	self.billing = ko.observable('<?php echo isset($data['KODE_BAYAR']) ? $data['KODE_BAYAR'] : '' ?>');
    ...
    self.uraian = ko.observable('<?php echo isset($data['URAIAN']) ? json_encode($data['URAIAN']) : '' ?>')
      .extend({
      ...
      success: function(res, xhr){
        if (res.isSuccess){
          ...
          if (res.nomor) App.nospt(res.nomor);
					if (res.billing) App.billing(res.billing);
            App.init_grid();
