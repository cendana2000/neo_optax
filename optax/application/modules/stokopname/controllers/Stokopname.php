...
class Stokopname extends Base_Controller
{
	...
	{
		$barang = $this->barang->select(array('filters_static' => varPost(), 'sort_static' => 'barang_kode asc'));
		$html = '';
	...
	public function select_ajax($value = '')
	{
		...
		$return = $this->db->query('SELECT barang_id as id, concat(barang_kode, " - ", barang_nama) as text, barang_stok as saved FROM v_pos_barang WHERE ' . $where . ' (barang_nama like "' . $data['q'] . '%" OR barang_kode like "' . $data['q'] . '%") ORDER BY barang_nama LIMIT ' . $data['page'] . $data['limit'])->result_array();
		$this->response(array('items' => $return, 'total_count' => $total[0]['total']));
	}
}
