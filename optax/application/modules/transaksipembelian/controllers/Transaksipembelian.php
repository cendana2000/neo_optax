...
class Transaksipembelian extends Base_Controller
{
	...
	{
		$this->response(
			$this->select_dt(varPost(), 'transaksipembelian', 'table')
		);
	}
	...
	public function barang_ajax($value = '')
	{
		...
		$return = $this->db->query("select barang_id as id, barang_kode, barang_nama, barang_harga, barang_kode as saved,
		...
		((select SUM(pembelian_detail_qty_barang) as stok_clean from pos_pembelian_barang_detail
		where pembelian_detail_barang_id = pb.barang_id) - SUM(ppd.penjualan_detail_qty_barang)) as barang_stok
		from pos_barang pb
		left join pos_penjualan_detail ppd 
			on pb.barang_id = ppd.penjualan_detail_barang_id
		inner join pos_jenis pj
			on pj.jenis_id = pos_barang.barang_jenis_barang
		WHERE pj.jenis_include_stok = 1
		group by ppd.penjualan_detail_barang_id, pb.barang_id")->result_array();
		$new_return = [];
		foreach ($return as $key => $value) {
			if ($value['barang_stok'] == NULL) {
				$value['barang_stok'] = $value['stok_clean'];
			}
			$new_return[] = [
				'id' 	=> $value['id'],
				'view' 	=> '<span class="detail-barang-select" style="width: 45px;">' . $value['barang_kode'] . '</span><span class="detail-barang-select"  style="width: 320px;">' . $value['barang_nama'] . '</span><span class="detail-barang-select" style="width: 100px;">' . number_format($value['barang_harga_beli']) . '</span><span class="detail-barang-select" style="width: 65px;">Stok : ' . $value['barang_stok'] . '</span>',
				'saved'	=> $value['saved'],
	...
	public function table_faktur()
	{
		...
		if ($filter != '') {
			// Query Lama
			// $this->response(["aaData" => $this->db->query("SELECT * FROM v_pos_pembelian_barang WHERE pembelian_bayar_sisa <> 0 AND supplier_kodes = '$filter'")->result_array()]);

			// Query Baru
			$data['aaData'] = $this->db->query("SELECT * FROM v_pos_pembelian_barang WHERE pembelian_supplier_id = '$filter' AND pembelian_bayar_opsi = 'K'")->result_array();
			...
			foreach ($detail['data'] as $key => $value) {
				...
				$html .= '<tr class="barang_' . $row . '">
						<td scope="row">
							...
							<input type="text" class="form-control barcode-scan" name="barang_kode[' . $row . ']" id="barang_kode_' . $row . '"  style="width:18%;display:inline;" data-id="' . $row . '" value="' . $value['barang_kode'] . '" readonly="">
				...
				$html .= '</select></td>
						...
						<td>							
							<div class="kt-input-icon kt-input-icon--right">
								<input type="text" class="form-control disc" id="pembelian_detail_diskon_' . $row . '" name="pembelian_detail_diskon[' . $row . ']" onkeyup="countRow(' . $row . ')" value="' . $value['pembelian_detail_diskon'] . '">
	...
	public function store()
	{
		...
		// $data['pembelian_user'] = $this->session->userdata('pegawai_id');
		$kode = 'T';
		...
		$data['pembelian_kode'] = $this->transaksipembelian->generate_kode($data['pembelian_tanggal'], $kode);
	...
	public function rekap_jurnal($trans)
	{
		$where = ['pembelian_tanggal' => $trans['jurnal_umum']];
		$jurnal = $this->db
			->select('SUM(pembelian_bayar_grand_total) as total')
			->where($where)
			->order_by('akun_kode', 'ASC')
			->group_by('akun_kode')
			->get('pos_pembelian_barang')
			->row_array();
	}
	...
	public function update()
	{
		...
		$operation = $this->transaksipembelian->update($data['pembelian_id'], $data, function ($res) use ($data, $delete, $last_detail) {
			...
			// //id => value
			// $debit = [
			// 	'5001' => $data['pembelian_bayar_grand_total']
			// ];
			// // '5001' => 'Barang Dagangan Tunai'
			// $debit_uraian = [
			// 	'5001' => 'Pembelian Barang Dagangan Tunai ' . $dt['supplier_nama']
			// ];
			// $kredit = [];
			// $bfr_diskon = $data['pembelian_bayar_grand_total'];
			// $trans = [
			// 	// 'jurnal_umum_nobukti' 			=> $this->jurnal->generate_kode('BKK', $dt['pembelian_tanggal']),
			// 	'jurnal_umum_total' 			=> $dt['pembelian_bayar_grand_total'],
			// 	'jurnal_umum_tanggal' 			=> $dt['pembelian_tanggal'],
			// 	'jurnal_umum_penerima' 			=> $data['penerima'],
			// 	'jurnal_umum_lawan_transaksi'   => $dt['pembelian_supplier_id'],
			// 	'jurnal_umum_keterangan'		=> 'Barang Dagangan Tunai',
			// 	'jurnal_umum_reference'			=> 'persediaan_barang',
			// 	'jurnal_umum_unit'				=> '1',
			// 	'jurnal_umum_reference_id'		=> $dt['pembelian_id'],
			// 	'jurnal_umum_reference_kode'	=> $dt['pembelian_kode'],
			// ];
			// $last_jurnal = $this->db
			// 	->select('jurnal_umum_id')
			// 	->where(['jurnal_umum_reference_id' => $dt['pembelian_id']])
			// 	->get('ak_jurnal_umum')
			// 	->row_array();
			// if ($data['pembelian_bayar_opsi'] == 'K') {
			// 	// $kredit['2112'] = $bfr_diskon;
			// 	// $trans['jurnal_umum_keterangan'] = 'Pembelian Kredit';
			// 	// $trans['jurnal_umum_reference_kode'] = '';
			// 	// $trans['jurnal_umum_penerima'] = 'Unit Eka Mart';

			// 	if ($last_jurnal) {
			// 		$debit = $kredit = $trans = $debit_uraian = $kredit_uraian = [];
			...
			// } else {
			// 	// $akun = $data['pembelian_akun_id'];
			// 	$kredit[$dt['pembelian_akun_id']] = $bfr_diskon;
			// 	$kredit_uraian[$dt['pembelian_akun_id']] = '';
			// 	// $last_jurnal = $this->jurnal->read(['jurnal_umum_reference_id' => $dt['pembelian_id']]);
			// 	if ($last_jurnal) {
			// 		$trans['jurnal_umum_id'] = $last_jurnal['jurnal_umum_id'];
			// 		$this->jurnal->edit_jurnal($debit, $kredit, $trans, $debit_uraian, $kredit_uraian);
			// 	} else {
			// 		$trans['jurnal_umum_nobukti'] = $this->jurnal->generate_kode('BKK', $dt['pembelian_tanggal']);
	...
	{
		$data = varPost();
		$data['pembelian_deleted_at'] = date("Y-m-d H:i:s");
		$operation = $this->transaksipembelian->update($data['id'], $data);
		$this->response($operation);
	}
	...
	public function cetak($value = '')
	{
		if ($value != '') {
			...
			$html .= '<table style="width:100%;">
				...
				<tr>
					<td>No. Transaksi : ' . ($data['pembelian_kode'] ? substr($data['pembelian_kode'], 7) : "-") . '</td>
			...
			foreach ($detail as $key => $value) {
				...
				$hpp = $sjumlah + $pajak;
				// $data['pembelian_detail_harga_barang'][$key] = $hpp/$data['pembelian_detail_konversi'][$key];
				$percentase = (($value['barang_harga'] - $hpp) / $hpp) * 100;
