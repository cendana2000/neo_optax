<script type="text/javascript">
		...
		HELPER.initTable({
			...
			columnDefs: [{
				...
				},
				{
					targets: 5,
					render: function(data, type, full, meta) {
						return full['pembelian_bayar_grand_total'];
					},
				},
				{
					targets: 6,
					render: function(data, type, full, meta) {
						return full['pembelian_retur'];
					},
				},
				{
					targets: 7,
					render: function(data, type, full, meta) {
						return full['pembelian_bayar_jumlah'];
					},
				},
				{
					targets: 8,
	...
	function countRow() {
		...
		$('.beli').each(function(i, v) {
			...
			$('#pembayaran_detail_sisa_' + nrow + ', #pembayaran_detail_bayar_' + nrow).val(sisa);
	...
	function addBeli(dt) {
		...
		html = `<tr class="beli beli_` + id + `" data-id="` + id + `">
					<td scope="row">
						...
						<input class="form-control" type="text" value="` + dt.pembelian_kode + `" name="pembelian_kode[]" id="pembelian_kode_` + id + `" disabled="">					
					</td>
					...
					<td><input class="form-control number" value="` + dt.pembayaran_detail_retur + `" type="text" name="pembayaran_detail_retur[` + id + `]" id="pembayaran_detail_retur_` + id + `" onkeyup="countRow('` + id + `')" value="` + dt.pembayaran_detail_retur + `" ></td>
					<td><input class="form-control number" value="` + dt.pembayaran_detail_potongan + `" type="text" name="pembayaran_detail_potongan[` + id + `]" id="pembayaran_detail_potongan_` + id + `" onkeyup="countRow('` + id + `')" value="` + dt.pembayaran_detail_potongan + `"></td>
					...
					<td><a href="javascript:;" data-id="` + id + `" class="btn btn-sm btn-danger btn-icon btn-icon-md kt-font-bold kt-font-warning" onclick="remRow('` + id + `')" title="Hapus" >
                  		<span class="la la-trash"></span> Hapus</a></td>
				</tr>`;
	...
	function onDelete(pembayaran_id) {
		HELPER.confirm({
			...
			callback: function(suc) {
				if (suc) {
					HELPER.ajax({
						url: BASE_URL + 'pembayaran/destroy',
						data: {
							id: pembayaran_id
						},
