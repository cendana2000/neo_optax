...
<script type="text/javascript">
	$(function() {
		HELPER.api = {
			index: BASE_URL + 'logpooling',
			get_parent: BASE_URL + 'logpooling/go_tree',
			ajax_toko: BASE_URL + 'logpooling/select_wp',
			deleteTransaksi: BASE_URL + 'logpooling/deleteTransaksi',
		}
		// $('.select2').select2();
			...
			columnDefs: [{
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						let html = '';
						let mstatus = {
							db: '<span class="label label-inline label-primary mr-2">Database</span>',
							api: '<span class="label label-inline label-warning mr-2">Api</span>',
							oapi: '<span class="label label-inline label-warning mr-2">OAPI</span>',
							excel: '<span class="label label-inline label-success mr-2">Excel</span>',
						}
						if (full['penjualan_source'] == 'db') {
							html = mstatus.db
						} else if (full['penjualan_source'] == 'Api') {
							html += mstatus.api
						} else if (full['penjualan_source'] == 'OAPI') {
							html += mstatus.oapi
						} else if (full['penjualan_source'] == 'excel') {
							html += mstatus.excel
						}
						return html;
					},
	...
	function getSpreadsheetTransaksiWp() {
		...
		HELPER.block();
		$.ajax({
			url: BASE_URL + '/logpooling/spreadsheet',
			type: 'post',
			data: {
				log_penjualan_code_store: $('#select_toko').val(),
				periode: $('#periode').val(),
