...
class Returpembelian extends Base_Controller {
		...
		$operation = $this->returpembelian->insert(gen_uuid($this->returpembelian->get_table()), $data, function($res) use ($data)
		{
			...
			foreach ($data['retur_pembelian_detail_barang_id'] as $key => $value) {
				$detail = [
					...
					'retur_pembelian_detail_tanggal' 		=> $data['retur_pembelian_tanggal'],
			...
			$total = $this->db->query('SELECT SUM(retur_pembelian_total) retur_total FROM pos_retur_pembelian_barang WHERE retur_pembelian_pembelian_id = "'.$data['retur_pembelian_pembelian_id'].'"')->row_array();
			if(!$total) $total = ['retur_total' => 0];
			// if($data['bayar-lunas'] !== '1'){
			$up_beli = $this->db->where('pembelian_id', $data['retur_pembelian_pembelian_id'])
				->set('pembelian_retur', $total['retur_total'])
	...
	public function destroy()
	{
		...
		$total = $this->db->query('SELECT SUM(retur_pembelian_total) retur_total FROM pos_retur_pembelian_barang WHERE retur_pembelian_pembelian_id = "'.$data['id'].'"')->row_array();
		if(!$total) $total = ['retur_total' => 0];
		// if($data['bayar-lunas'] !== '1'){
		$up_beli = $this->db->where('pembelian_id', $data['id'])
			->set('pembelian_retur', $total['retur_total'])
			->update('pos_pembelian_barang');
		$this->response($operation);
	}
