<script type="text/javascript">
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						...
						if (HELPER.get_role_access('User-Read')) {
							btn_aksi += `<a href="javascript:;" class="btn btn-sm btn-primary" onclick="onDetail('` + full['user_id'] + `')">\
											Detail
										</a> `;
						}
						menu = ""
						...
						if (HELPER.get_role_access('User-Update')) {
							menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onEdit('` + full['user_id'] + `')"><i class="nav-icon fa fa-pen"></i><span class="nav-text">Edit</span></a></li>`;
							// menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onSetProject('` + full['user_id'] + `')"><i class="nav-icon fa fa-pen"></i><span class="nav-text">Set Project</span></a></li>`;
						}
						if (HELPER.get_role_access('User-Delete')) {
							menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onDelete('` + full['user_id'] + `')"><i class="nav-icon fa fa-trash"></i><span class="nav-text">Delete</span></a></li>`;
						}
						if (HELPER.get_role_access('User-ResetPassword')) {
							menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onResetPassword('` + full['user_id'] + `')"><i class="nav-icon fa fa-undo"></i><span class="nav-text">Reset Password</span></a></li>`;
						}
						if (HELPER.get_role_access('User-Status')) {
							status = full['user_status'];
							if (status == 1) {
								menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onStatus('` + full['user_id'] + `')"><i class="nav-icon fa fa-times"></i><span class="nav-text">Set Non Active</span></a></li>`;
							} else {
								menu += `<li class="nav-item"><a class="nav-link" href="javascript:;" onclick="onStatus('` + full['user_id'] + `')"><i class="nav-icon fa fa-check"></i><span class="nav-text">Set Active</span></a></li>`;
							}
						}
						if (show_dropdown) {
	...
	function onCreate() {
		...
		$('#user_foto').css('background-image', 'url(./assets/media/noimage.png)');
	...
	function onDetail(user_id) {
		HELPER.ajax({
			...
			complete: function(res) {
				...
				if (res.user_foto) {
					img_full = BASE_CONTENT + 'user/' + res.user_foto;
					img = BASE_CONTENT + 'user/thumbs/' + res.user_foto;
				} else {
