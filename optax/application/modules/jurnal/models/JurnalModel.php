...
class JurnalModel extends Base_Model {
		...
		$model = array(
			'table' => array(
				...
				'fields' => array(
					...
					array('name' => 'jurnal_umum_keterangan', 'htmlspecial' => true),
	...
	public function generate_kode($prefix= 'BU', $month){
		...
		$kode = $this->db->query('SELECT jurnal_umum_nobukti FROM ak_jurnal_umum WHERE jurnal_umum_nobukti like "%'.$bulan.'-'.$prefix.'%"  AND jurnal_umum_reference not like "%delete" order by jurnal_umum_tanggal DESC,jurnal_umum_nobukti desc limit 1')->result_array();
	...
	public function edit_jurnal($debit = [], $kredit = [], $trans = [], $debit_keterangan = [], $kredit_keterangan = [])
	{
		...
		$detail = $this->db->select('jurnal_umum_detail_akun, jurnal_umum_detail_tipe,  jurnal_umum_detail_total, jurnal_umum_detail_id')
				 ...
				 ->result_array();
		$temp_detail = [];
		$trans_detail = (strpos($trans['jurnal_umum_nobukti'], 'BK') !== false ? 'kas':'umum');
		foreach ($detail as $key => $value) {
			$this->db->delete('ak_jurnal_umum_detail', ['jurnal_umum_detail_id' => $value['jurnal_umum_detail_id']]);
			$this->akunsaldo->update_saldo(array(
			...
			if(is_array($value)){
				foreach ($value as $k => $v) {
					...
					$qdebit[] = [
						...
						'jurnal_umum_detail_transaksi'	=> $trans_detail,
					];
					$tdebit += $v;
					$no++;
				}
		...
		if(isset($trans['jurnal_umum_akun_id'])) $qjurnal['jurnal_umum_akun_id'] = $trans['jurnal_umum_akun_id'];
