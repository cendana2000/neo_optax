		...
		$model = array(
			'table' => array(
				...
				'fields' => array(
					...
					array('name' => 'kartu_created_at'),
					array('name' => 'kartu_updated_at'),
					array('name' => 'kartu_updated_at'),
			...
			if ($data['kartu_transaksi'] == 'Opname') {
				...
				if ($selisih !== 0) {
					...
					$update->set('kartu_stok_akhir', 'kartu_stok_akhir+' . $selisih, FALSE)
						...
						->set('kartu_updated_at', date('Y-m-d H:i:s'))
						->set('kartu_updated_at', $this->session->userdata('user_id') . '-' . $this->session->userdata('pegawai_nama'))
