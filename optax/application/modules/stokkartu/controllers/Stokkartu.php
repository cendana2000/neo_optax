...
class Stokkartu extends Base_Controller
{
	...
	public function barang_ajax($value = '')
	{
		...
		$return = $this->db->query('SELECT barang_id as id, barang_kode, barang_nama, barang_harga, barang_stok, barang_kode as saved FROM v_pos_barang WHERE ' . $where . ' (barang_nama like "' . $data['q'] . '%" OR barang_kode like "' . $data['q'] . '%") ORDER BY barang_nama LIMIT ' . $data['page'] . $data['limit'])->result_array();
		// , " (stok: ", barang_stok, ")"
		$new_return = [];
