					...
					$.each(res, (v,data) => {
						...
						$('#pesanan').append(`
							<tr>
								...
								<td>`+moment(data.anggota_tgl_pesan).format("DD MMMM YYYY HH:mm")+`</td>
		...
		$.ajax({
			...
			success : (res) => {
				...
				$.each(res, (v,data) => {
					...
					anggota_id = data.anggota_id+data.anggota_kode+' - '+data.anggota_nama;
				});
				$('#subtotal').html('Rp. '+($.number(subtotal)))
				$('#button-proses').html(`<button type="button" class="btn btn-sm btn-primary float-right" onclick="prosesPesanan('`+anggota_id+`')">Proses</button>`);
			}
		})
	}
	...
	function prosesPesanan(id){
		Swal.fire({
			...
			if(res.value){
				...
				addPesanan(anggota_id.substr(0,4));
				$("#penjualan_anggota_id").select2("trigger", "select", {
					data: {
						id: anggota_id.substr(0,4),
						text: anggota_id.substr(5)
					}
				});
				// $.ajax({
		...
		$.ajax({
			...
			success : (res) => {
				...
				$('#table-detail_barang').append(`<tr class="barang_` + row + `">
					...
					<td><select style="background: #5d78ff!important; color :white;" class="form-control" name="penjualan_detail_satuan[` + row + `]" id="penjualan_detail_satuan_` + row + `" style="width: 100%" ></select>
						<input type="hidden" class="form-control" name="penjualan_detail_satuan_kode[` + row + `]" id="penjualan_detail_satuan_kode_` + row + `" >						
					</td>					
					<td><input style="background: #5d78ff!important; color :white;" class="form-control number" type="text" name="penjualan_detail_harga[` + row + `]" id="penjualan_detail_harga_` + row + `" readonly=""></td>
					<td>
						<input style="background: #5d78ff!important; color :white;" class="form-control number qty" type="text" name="penjualan_detail_qty[` + row + `]" id="penjualan_detail_qty_` + row + `" onkeyup="countRow('` + row + `')" value="`+data.barang_qty+`">
						<input class="form-control number" type="hidden" name="penjualan_detail_qty_barang[` + row + `]" id="penjualan_detail_qty_barang_` + row + `">						
					</td>
					<td>
						<input style="background: #5d78ff!important; color :white;" class="form-control disc" type="text" name="penjualan_detail_potongan_persen[` + row + `]" id="penjualan_detail_potongan_persen_` + row + `" onkeyup="countRow('` + row + `')">
						<input class="form-control number" type="hidden" name="penjualan_detail_potongan[` + row + `]" id="penjualan_detail_potongan_` + row + `">
					</td>
					<td><input style="background: #5d78ff!important; color :white;" class="form-control number jumlah" type="text" name="penjualan_detail_subtotal[` + row + `]" id="penjualan_detail_subtotal_` + row + `" readonly=""></td>
					<td style="text-align: center;"><a href="javascript:;" data-id="` + row + `" class="btn btn-sm btn-clean btn-icon btn-icon-md kt-font-bold kt-font-warning" onclick="remRow(this)" title="Hapus" >
                  		<span class="la la-trash"></span> Hapus</a></td>
				</tr>`);
				setSatuan(row)
