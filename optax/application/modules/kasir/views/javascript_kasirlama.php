<script type="text/javascript">
    ...
    $(function() {
        ...
        let saldo_voucher;
        $('#pembelian_diskon').number(true);
        $('#pembelian_pajak').number(true);
							...
							<div class="card cardCustom" style="height:100px">
								<div class="m-4" style="height:100% !important;">
									<div class="row" style="height:90% !important;">
										...
										<div class="col-4">
										<button onclick="initOrder('${param.id}', '${param.barang_nama}', '${param.barang_harga}');" class="btn btn-icon btn-circle" style="background-color: white; color:var(--color1); "><i class="fas fa-plus"></i></button>
										</div>
										</div>
										<div class="row pb-3">
											<div class="col-12">
												<span style="font-size:11px; ">Rp. ${$.number(param.barang_harga)}</span>
											</div>
									</div>
								</div>
							</div>
    ...
    function initOrder(id, barang_nama, barang_harga) {
        event.preventDefault();
        let qty = 1;
        ...
        let html = `
			<tr class="order_${row}">
				...
				<td>
					...
					<input type="number" value="${qty}" id="penjualan_detail_qty_barang_${row}" onchange="countPrice(${row}, ${barang_harga})" name="penjualan_detail_qty_barang[${row}]" class="form-control qty">
				</td>
				<td>
        ...
        if (flag < 1) { // jika belum ada data dalam table
            ...
            countPrice(cIndex, barang_harga);
        }
        countDiscount();
        countKembalian();
        countQty();
    ...
    function countPrice(cRow, cHarga) {
        event.preventDefault();

        let cQty = $('#penjualan_detail_qty_barang_' + cRow).val();
        let cTotal = cQty * cHarga;
        // $('#penjualan_detail_harga_beli_' + cRow).text(cTotal);
    ...
    function countPajak() {
        ...
        let inputPajak = $('#pembelian_pajak').val();
    ...
    function countDiscount() {
        ...
        let inputDiscount = $('#pembelian_diskon').val();
        ...
        HELPER.initTable({
            ...
            columnDefs: [{
                ...
                },
                {
                    targets: 4,
                    render: function(data, type, full, meta) {
                        return full['penjualan_total_harga'] + full['penjualan_total_potongan'];
                    },
                },
                {
                    targets: 5,
                    render: function(data, type, full, meta) {
                        return full['penjualan_total_potongan'];
                    },
                },
                {
                    targets: 6,
                    render: function(data, type, full, meta) {
                        return full['penjualan_total_harga'];
                    },
                },
                {
                    targets: 7,
        ...
        HELPER.loadData({
            ...
            callback: function(res) {
                ...
                $.each(res.detail.data, function(i, v) {
                    barang.push('#penjualan_detail_barang_id_' + row);
