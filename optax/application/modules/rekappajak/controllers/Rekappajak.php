...
class Rekappajak extends Base_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'rekappajak/rekappajakparentModelV5' => 'realisasiparent',
			'rekappajak/rekappajakparentfilterModelV8' => 'realisasiparentfilter',
	...
	public function get_kecamatan()
	{
        ...
        ')
			...
			->join('conf_pemda cp', 'pp.pemda_id = cp.pemda_id', 'left')
			->join('conf_kecamatan ck', 'ck.kabkota_id = cp.kabkota_id', 'left');

		if ($this->session->userdata('login_access') == 'pemda') {
			$where = [
				'pp.pegawai_id' => $this->session->userdata('user_pegawai_id'),
				'pp.pemda_id'   => $this->session->userdata('pemda_id')
			];
		} else {
			$where = [
				'cp.kabkota_id' => $data['kabkota_id']
			];
		}

		$this->db->where($where);
		$this->db->group_by(['ck.kecamatan_kode', 'ck.kecamatan_nama']);
